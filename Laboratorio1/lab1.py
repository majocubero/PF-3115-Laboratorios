...
def main(datafile):
    ...
    w, b, loss = training(2000, torch.tensor(1), torch.tensor(1), float(0.0009), tensor3_AL, tensor3_APM)
    #show_results_table(w, b, loss)
    
    print("\n---Normalized result---\n")
    xn = get_xn(tensor3_AL)
    w, b, loss= training(10000, torch.tensor(0), torch.tensor(300), float(0.00001), xn, tensor3_APM)
    show_results_table(w, b, loss)
    ...
    print("\n---Traninig auto (X)---\n")
    w, b, loss= training_auto(2000, torch.tensor(float(1)), torch.tensor(float(1)), float(0.1), tensor3_AL, tensor3_APM)
    show_results_table(w, b, loss)
    
    print("\n---Traninig auto (Xn)---\n")
    w, b, loss= training_auto(10000, torch.tensor(float(0)), torch.tensor(float(225)), float(0.00001), xn, tensor3_APM)
    show_training_plot(w, b, tensor3_AL, tensor3_APM, " training auto set (Xn): 80%", str(loss[-1].item()))
    show_results_table(w, b, loss)
    
    print("\n---Traninig opt SGD (Xn)---\n")
    w, b, loss= training_opt_SGD(10000, torch.tensor(float(0)), torch.tensor(float(225)), float(0.00001), xn, tensor3_APM)
    ...
    w, b, loss= training_opt_ADAM(10000, torch.tensor(float(0)), torch.tensor(float(225)), float(0.00001), xn, tensor3_APM)
